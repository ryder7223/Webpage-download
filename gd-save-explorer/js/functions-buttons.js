...
function loadButtons() {
    ...
    $('#saveLevelFilters').click(function() {
        ...
        if (selectedFolder > 0) fs = fs.filter(x => x.folder == selectedFolder)
        ...
        $('.minFilter').each(function() {
            ...
            if (min && Number.isInteger(+min)) fs = fs.filter(x => (x[filterName] || 0) >= +min)
            if (max && Number.isInteger(+max)) fs = fs.filter(x => (x[filterName] || 0) <= +max)
        })
        ...
        Object.keys(sortStuff.type).forEach(x => {
            if (!sortStuff.type[x]) fs = fs.filter(y => y.type != x)
        })

        Object.keys(sortStuff.stars).forEach(x => {
            if (!sortStuff.stars[x]) fs = fs.filter(y => y.type != "Official" && (y.stars || 0) != x)
        })

        Object.keys(sortStuff.demon).forEach(x => {
            if (!sortStuff.demon[x]) {
                if (x == 1) fs = fs.filter(y => y.demonType && y.demonType > 1)
                else fs = fs.filter(y => y.demonType != x)
            }
        })

        Object.keys(sortStuff.userRating).forEach(x => {
            if (sortStuff.userRating[x]) fs = fs.filter(y => y[x])
        })

        if (sortStuff.uncompletedFilter.stars) fs = fs.filter(y => y.stars > 0 && y.percentage < 100)
        if (sortStuff.uncompletedFilter.orbs) fs = fs.filter(y => y.stars > 1 && y.manaOrbPercentage < 100)
        if (sortStuff.uncompletedFilter.leaderboard) fs = fs.filter(y => y.leaderboardPercentage < 100)
    ...
    $('#downloadXML').click(function() {
        return downloadFile(GameManagerXML, data.username + ".xml", !includePass)
    })

    $('#downloadJSON').click(function() {
        let dataJSON = { ...data } // clone
        dataJSON.allLevels = levels
        return downloadFile(JSON.stringify(dataJSON, null, 2), data.username + ".json")
    })

    $('#downloadLevels').click(function() {
        return downloadFile(JSON.stringify(levels, null, 2), data.username + "_levels.json")
    })
